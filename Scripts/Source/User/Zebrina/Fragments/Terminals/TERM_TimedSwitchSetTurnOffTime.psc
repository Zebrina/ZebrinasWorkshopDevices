...
Scriptname Zebrina:Fragments:Terminals:TERM_TimedSwitchSetTurnOffTime Extends Terminal Hidden Const
...
function SetTimeParam(float afGameHour, ObjectReference akTerminalRef)
    if (akTerminalRef)
        ObjectReference[] configRefs = akTerminalRef.getLinkedRefArray(LinkTerminalTimedSwitch)
    	int i = 0
    	while (i < configRefs.length)
			(configRefs[i] as Zebrina:Workshop:TimedSwitchScript).fTurnOffAtGameHour = AfGameHour
    		i += 1
    	endWhile
    else
        (Game.GetPlayer().GetLinkedRef(WorkshopLinkObjectConfiguration) as Zebrina:Workshop:TimedSwitchScript).fTurnOffAtGameHour = AfGameHour
    endif
    ConfigTerminalValue2.SetValue(AfGameHour)
endfunction

Keyword property LinkTerminalTimedSwitch auto const mandatory
