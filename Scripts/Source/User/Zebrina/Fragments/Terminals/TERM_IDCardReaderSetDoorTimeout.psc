...
Scriptname Zebrina:Fragments:Terminals:TERM_IDCardReaderSetDoorTimeout Extends Terminal Hidden Const
...
function SetButtonTimeout(float afValue)
    ...
    if (configureObjectRef != none && configureObjectRef is Zebrina:Workshop:IDCardReaderScript)
        (configureObjectRef as Zebrina:Workshop:IDCardReaderScript).fTimeoutSeconds = afValue
        ConfigTerminalCurrentValue1.SetValue(afValue)
    endif
endfunction
