...
Scriptname Zebrina:Fragments:Terminals:TERM_NighttimeSwitchSetEndTime Extends Terminal Hidden Const
...
function SetTimeParam(float gameHour, ObjectReference terminalRef)
    if (terminalRef)
        ...
        if (configureObjectRef)
            configureObjectRef.fTurnOffAtGameHour = gameHour
            if (configureObjectRef.fTurnOnAtGameHour == gameHour)
                if (gameHour == 0.0)
                    configureObjectRef.fTurnOnAtGameHour = 23.0
                else
                    configureObjectRef.fTurnOnAtGameHour = gameHour - 1.0
                endif
            endif
