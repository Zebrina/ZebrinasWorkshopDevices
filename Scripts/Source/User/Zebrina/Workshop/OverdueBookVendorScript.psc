...
function UpdateTotalItemCountAndAliases()
    ...
    if (lastInventoryReset < lastGlobalInventoryReset)
        ...
        int i = 0
        while (i < PrizeItemAliases.Length && self.ItemsInMachine.Length <= 10)
            if (PrizeItemAliases[i].GetReference() != none)
                Zebrina:Workshop:OverdueBookVendorPrizeItem prizeItem = PrizeItemAliases[i] as Zebrina:Workshop:OverdueBookVendorPrizeItem
                self.ItemsInMachine.Add(prizeItem.ConvertToItemAndCount())
            endif
            i += 1
        endwhile
    ...
    ;Debug.MessageBox("UpdateTotalItemCountAndAliases")
