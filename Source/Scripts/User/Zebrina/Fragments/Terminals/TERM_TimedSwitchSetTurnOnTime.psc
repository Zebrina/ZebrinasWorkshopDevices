...
Scriptname Zebrina:Fragments:Terminals:TERM_TimedSwitchSetTurnOnTime Extends Terminal Hidden Const
...
function SetTimeParam(float AfGameHour, ObjectReference akTerminalRef)
    if (akTerminalRef)
    	...
    	while (i < configRefs.length)
			(configRefs[i] as Zebrina:Workshop:TimedSwitchScript).fTurnOnAtGameHour = AfGameHour
    		i += 1
    	endWhile
    else
        (Game.GetPlayer().GetLinkedRef(WorkshopLinkObjectConfiguration) as Zebrina:Workshop:TimedSwitchScript).fTurnOnAtGameHour = AfGameHour
    endif
    ConfigTerminalValue1.SetValue(AfGameHour)
endfunction
